...
//! ## Subscriber setup
//!
//! To see flattened top-level JSON fields, configure `tracing-subscriber` with `.json()` and
//! `.flatten_event(true)`. If you also want the inherited `context` map included automatically,
//! enable the `with_context` feature on the `log-args-runtime` crate in your `Cargo.toml`:
//!
//! ```toml
//! [dependencies]
//! log_args = { path = "./log_args" }
//! log-args-runtime = { path = "./log-args-runtime", features = ["with_context"] }
//! tracing = "0.1"
